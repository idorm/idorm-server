...
public record MatchingInfoRequest(
    ...
    @Size(max = 30, message = "wakeupTime, 기상 시간은 ~30자 이내여야 합니다.")
    String wakeupTime,

    @NotBlank(message = "정리정돈 상태 입력은 필수입니다.")
    @Size(max = 30, message = "cleanUpStatus , 정리정돈 상태는 ~30자 이내여야 합니다.")
    String cleanUpStatus,

    @NotBlank(message = "샤워 시간 입력은 필수입니다.")
    @Size(max = 30, message = "showerTime, 샤워시간은 ~30자 이내여야 합니다.")
    String showerTime,

    @NotBlank(message = "오픈 채팅 링크 입력은 필수입니다.")
    @Size(max = 100, message = "openKakaoLink , 오픈채팅 링크는 ~100자 이내여야 합니다.")
    String openKakaoLink,

    @Size(max = 4, message = "mbti , mbti는 ~4자 여야 합니다.")
    String mbti,

    @Size(max = 150, message = "wishText , 하고싶은 말은 ~150자 이내여야 합니다.")
    String wishText
) {
